...
# X_train_3 = autoencoder(X_train[cut_2:], stop=True).detach().numpy()
...
# Kmeans_index = clust_method.fit_predict(X_train)
...
# X_train_predict = np.concatenate((X_train_2, X_train_3), axis=0)

time1 = round(time.time()*1000)
PSC_index_2 = psc.predict(X_train_2)
# PSC_index = np.concatenate((PSC_index_1, PSC_index_2), axis=0)
time2 = round(time.time()*1000)
print(f"PSC time spent: {time2 - time1} milliseconds")
...
print("\nsc")
time1 = round(time.time()*1000)
Kmeans_index = sc.fit_predict(X_train)
time2 = round(time.time()*1000)
print(f"Kmeans time spent: {time2 - time1} milliseconds")
